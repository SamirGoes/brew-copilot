...
- **THEN** system calculates and displays the IBU contribution (approximately 25 IBU)
...
- **WHEN** original recipe calls for 30g at 60min for 25 IBU, but user will boil for only 45min
- **THEN** system calculates new hop amount needed to achieve same 25 IBU

#### Scenario: Longer boil time allows less hops
- **WHEN** original recipe calls for 30g at 60min for 25 IBU, but user will boil for 90min
- **THEN** system calculates reduced hop amount to avoid exceeding target IBU
...
- **THEN** system displays utilization percentage (approximately 25%)
